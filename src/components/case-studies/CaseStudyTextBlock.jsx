...
export default function CaseStudyTextBlock ({ content }) {
    ...
    const references = content.body?.references || []
      ...
      const richTextOptions = {
        ...
        renderNode: {
          // 1) Replace <h3> blocks with our NumberedH3
      ...
      [BLOCKS.EMBEDDED_ASSET]: (node) => {
        ...
        const matchedAsset = references.find(
          (ref) => ref.__typename === "ContentfulAsset" && ref.contentful_id === assetId
        )
      ...
      [BLOCKS.EMBEDDED_ENTRY]: (node) => {
        ...
        const matchedEntry = references.find(
          (ref) => ref.__typename === "ContentfulQuote" && ref.contentful_id === entryId
        )
    ...
    return (
        <TextBlockSection>
            ...
            {/* non-embedded quote */}
        {quoteRef && (
            <CaseStudyQuoteBlock quote={quoteRef} />
        )}
        </TextBlockSection>
    )
}